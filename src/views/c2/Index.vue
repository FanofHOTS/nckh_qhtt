    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 my-6">
      <div class="bg-white shadow-md p-5">
        ...
        <div class="text-center md:text-left" v-if = "!isHidden">
          ...
          <button @click="isHidden = true" id="D1" class="bg-green-500 text-white hover:bg-green-600 focus:outline-none rounded px-4 py-2 mr-2">
            Dạng 1
          </button>
        ...
        <div class="text-center md:text-left" v-if = "isHidden">
          ...
          <button @click="isHidden = false" id="D2" class="bg-green-500 text-white hover:bg-green-600 focus:outline-none rounded px-4 py-2 mr-2">
            Dạng 2
          </button>
              ...
              <table class="table-auto text-center w-full mb-5">
                  <thead>
                    <tr>
                      ...
                      <th class="border px-4 py-2" v-for="x in output.nX" :key="'th' + x">x<sub>{{ x }}</sub></th>
                    </tr>
                  </thead>
                      <tbody v-for="(step, index) in output.steps" :key="index" class="border">
                          <tr v-for="(row, i) in step" :key="'row' + i">
                            <template v-if="i < output.nLine">
                                <td class="border-l border-r px-4 py-2" v-for="(val, j) in row" :key="'td' + j + '-' + i">
                                  <div v-if = "j === 1">
                                    x<sub>{{ val }}</sub>
                                  </div>
                                  <div v-else>
                            ...
                            <template v-else>
                                ...
                                <td class="border px-4 py-2"></td>
                                <td class="border px-4 py-2"></td>
                                <td class="border py-3" v-for="(val, j) in row" :key="'td_' + j + '-' + i">{{ val }}</td>
                            </template>
                          </tr>
                      </tbody>
              </table>
                      ...
                      <tbody v-for="(step, index) in output.steps" :key="index" class="border">
                          <tr v-for="(row, i) in step" :key="'row' + i">
                            ...
                            <template v-else>
                                ...
                                <td class="border px-4 py-2"></td>
                                <td class="border px-4 py-2"></td>
                                <td class="border-l border-r border py-3" v-for="(val, j) in row" :key="'td_' + j + '-' + i">{{ val }}</td>
                            </template>
                          </tr>
                      </tbody>
                  </table>
