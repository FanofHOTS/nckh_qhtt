<template>
  <div>
    <h2 class="text-2xl font-semibold text-center md:text-left">Chương 4</h2>
    <p class="text-gray-700 text-center md:text-left">Ứng dụng QHTT: bài toán vận tải</p>
            ...
            <table class="table-auto text-center w-full">
              <tbody>
                <tr>
                  ...
                  <td v-for="(t, index) in output.thu" :key="'tr_thu_' + index" class="border px-4 py-2">{{ t }}</td>
                </tr>
                    <tr v-for="(p, index) in output.phat" :key="'tr_phat_' + index">
                      <td class="border px-4 py-2">{{ p }}</td>
                        <td v-for="(i, idx) in output.cp[index]" :key="'td_' + p + '_' + idx" class="border">
